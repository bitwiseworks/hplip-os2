...
def get_comment(package, Inst_status, installed_ver):
    ...
    if package == 'pyqt' or package == 'pyqt4':
        ...
        if Inst_status == 'OK':
            comment = "'hpaio found in /@unixroot/etc/sane.d/dll.conf'"
        else:
            comment = "'hpaio not found in /@unixroot/etc/sane.d/dll.conf. hpaio needs to be added in this file.'"
...
#        return self.missing_user_grps
    ...
    def validate(self,time_flag=DEPENDENCY_RUN_AND_COMPILE_TIME, is_quiet_mode= False):
        ...
        if v:
            ...
            log.info()
            log.info(log.bold("Current contents of '/@unixroot/etc/hp/hplip.conf' file:"))
            try:
                output = open('/@unixroot/etc/hp/hplip.conf', 'r').read()
            ...
            log.info()
            log.info(log.bold("Current contents of '/@unixroot/var/lib/hp/hplip.state' file:"))
            try:
                output = open(os.path.expanduser('/@unixroot/var/lib/hp/hplip.state'), 'r').read()
            except (IOError, OSError) as e:
            ...
            if device_avail:
                ...
                if cups_printers:
                    ...
                    for p in cups_printers:
                        ...
                        ppd = os.path.join('/@unixroot/etc/cups/ppd', printer_name + '.ppd')
