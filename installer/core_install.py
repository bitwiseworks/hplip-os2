...
FILES_LIST = ['/@unixroot/usr/share/cups/drv/hp/', '/@unixroot/usr/local/share/ppd/HP/', '/@unixroot/usr/local/share/cups/drv/hp/', '/@unixroot/usr/share/applications/hplip.desktop', '/@unixroot/usr/share/applications/hp-uiscan.desktop',
             '/@unixroot/etc/xdg/autostart/hplip-systray.desktop', '/@unixroot/etc/hp/hplip.conf', '/@unixroot/usr/share/doc/hplip-*', '/@unixroot/usr/lib/systemd/system/hplip-printer*.service']
        ...
        if (self.distro_name.lower()=='fedora') and (self.distro_version>='30'):
            ...
            return check_ps(['dbus-daemon'])  and \
                len(locate_file_contains("dbus-message.h",
                                         '/@unixroot/usr/include', 'dbus_message_new_signal'))
...
'''
        ...
        if pat.match(home_dir) is not None:
            ...
            if os.path.exists(usrlib_dir + 'sane/libsane-hpaio.so.1'):
                ...
                try:
                    ...
                    log.error(
                        "'/@unixroot/etc/sane.d/dll.conf' not found. Creating dll.conf file")
    ...
    def post_build(self, distro_ver=None):
        ...
        if self.reload_dbus and self.selected_options['policykit']:
            cmds.append(self.passwordObj.getAuthCmd() %
                        "sh /@unixroot/etc/init.d/dbus reload")
